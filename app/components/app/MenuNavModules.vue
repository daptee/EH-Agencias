    ...
    <v-list-item-group v-model="openGroups" mandatory color="primary">
      <template v-for="(item, i) in items" :key="`module_${i}`">
        ...
        >
          <template #activator>
            <v-list-item>
              ...
              <v-list-item-icon>
                <v-icon size="32">{{ openGroups[i] ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
              </v-list-item-icon>
            </v-list-item>
          </template>
        ...
        <v-list-item
          ...
          :class="[isMiniOpen ? '' : 'bodycolor', router.currentRoute.value.fullPath === `/${item.pathName}` ? 'v-item--active' : '']"
...
const items = ref<MenuItem[]>([
  ...
  { text: 'Mis reservas', pathName: 'mis-reservas', arrow: '$arrow_big', id: 1 },
])

const openGroups = ref<boolean[]>(items.value.map(() => false))
const isMiniOpen = computed(() => uiStore.isNavMini)
const userModules = computed<number[]>(() => [1]) // ejemplo
...
.item-icon-arrow-despl {
  ...
  .v-list-item {
    ...
    .v-list-item__content {
      ...
      min-width: 100%;
      .v-list-item-title {
        @include fontStyle(bold, 14px, 0.01em, $greytextdark);
      }
    }
    .v-list-item__icon {
      .item-left-icons {
    ...
    &:hover {
      ...
      .v-icon {
        svg > * {
          fill: $greytextdark !important;
        }
      }
    }
    &.v-item--active {
      background-color: $greytextdark !important;
